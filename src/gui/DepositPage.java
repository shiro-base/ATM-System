...
public class DepositPage extends JFrame implements ActionListener{
	...
	DepositPage()  {
		...
		label = new JLabel("Enter Deposit details : ");
		...
		try {
			atm = new ATM();
			choices = atm.getCustomerAccount();
			if(choices.isEmpty()){
		...
		amt = new JLabel("Enter Amount to Deposit");
				...
				try {
					...
					for ( Account<? extends Currency> e: choices) {
						if(String.valueOf(e.getAccountId()).equals(accSel)){
							String checkedResult = atm.deposit(e, tamt1);
